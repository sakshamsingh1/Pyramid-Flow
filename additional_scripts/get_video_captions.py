...
def test():
    ...
    for info in tqdm(metas):
        video_path = os.path.join(video_dir, info)
        video_data = open(video_path, 'rb').read()
        response = predict(prompt, video_data, temperature)
        vid = os.path.basename(video_path).replace('.mp4', '')
        caption_dict[vid] = response

    with open(save_path, 'w') as f:
        json.dump(caption_dict, f)

    # video_data = open('/home/sxk230060/TI2AV/Pyramid-Flow/text_to_video_sample.mp4', 'rb').read()
    # response = predict(prompt, video_data, temperature)
